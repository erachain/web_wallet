function getMail(params) {
    ...
    httpRequest('GET', "/apirecords/get/" + params.signature, function (data) {
        ...
        if (mailData.encrypted) {
            ...
            $('#inboxMailContent').append('<a href="javascript:" class="button__mail" onclick="mail_encrypted(\'' + mailData.data + '\', \'' + public_key + '\')">Decrypted</a>')
        } else {
            document.getElementById('mail-table-message').innerHTML = mailData.data;
        }
    })
}
...
function mail_encrypted(message, public_key) {
    document.getElementById('mail-table-message').innerHTML = EraCrypt.decryptMessage(message, public_key, appData.keyPair.privateKey);
