	...
	function sendAssetDoPaymentTransaction(type = null) {
		$('#base58SenderAccountAddress').html(appData.base58AccountAddress);
		// sender address
		...
		$("#txRaw").val(Base58.encode(generate_R_Send_TransactionBase(appData.keyPair, recipientAccountAddress, asset_key, quantity, scale, timestamp, title, message, encrypt, is_text, port)));
		askPOSTProcess(type);
	}
