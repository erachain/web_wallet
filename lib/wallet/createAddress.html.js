	...
	"WEB URL:<br>" +
		...
		"<a href='javascript:' id='server-list__link' class='server-list__link' onclick='show_server_list(this)'>ServerList: (show)</a>" +
		"<div id='server-list-data'></div>" +
		"</div>" +
		"<div class='form-check' style='margin-left:30px;' id='localStoreDiv'>" +
  			//"<input class='form-check-input' type='checkbox' value='' id='localStore'>" +
			 // "<label class='form-check-label' for='localStore'>Save private key at the local store</label>" +
		"</div><br>" +
	...
	"<div class='tab-pane active' id='wallet'><br>" +
		...
		"Base Seed:<br>" +
		"<div class='input-group'><input class='form-control' type='text' size=60 value='' id='base58BaseSeedWallet'/ autocomplete='off'>&nbsp;&nbsp;<input class='btn btn-default' type='button' id='doBrainButton' value='generate seed by BIP39' OnClick='generateSeedBIP39()'/></div><br>" +	
		"Accounts quantity:<br>" +
		"<input class='form-control' style='width:150px;' type='text' size=20 value='3' id='base58BaseSeedWalletQty'/><br>" +	
	...
	"<b>generate BIP39</b> - генерирует в поле passcode string набор слов на английском языке согласно спецификации Bitcoin BIP39. Набор может быть записан человеком без ошибок, в отличие от сгенерированного секретного ключа.<br>"+
	"<b>get Account Seed</b> - из поля passcode string вычисляет account seed. Из одинакового passcode всегда будет получаться одинаковый seed.<br>"+
	"WEB URL: по умолчанию сервер для разработчиков (http://erachain.org:9067), для боевой версии нужно указать http://erachain.org:9047 и нажать кнопку Save.<br>"+
	"<b>Save</b> - сохраняет в кошельке указанную в поле WEB URL ссылку на API Erfachain.<br>"+
	"<b>get Private key</b> - по введенному account seed генерирует privat key.<br>"+
	"<b>Log in</b> - сохраняет в кошельке privat key для использования функций кошелька.<br>"+
	"<b>get Address</b> - преобразует public key в address.<br><br>"+
