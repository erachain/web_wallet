        ...
        if (httpRequest.readyState === XMLHttpRequest.DONE && this.status === 200) {
            try {
                ...
                if (data_preview.title !== undefined) {
                    ...
                    if(data_preview.vouches_table !== undefined) {
                        let vouches_table = data_preview.vouches_table;
                        vouches_table = vouches_table.replace(/href=\?/g, 'target="_blank" href=' + appData.nodeUrl + '/index/blockexplorer.html?');
                        html += '<div class="transaction-preview__vouches">' + vouches_table+ '</div>';
                    }
