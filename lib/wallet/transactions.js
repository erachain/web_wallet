        ...
        try {
            httpRequest.onreadystatechange = function() {
                if (httpRequest.readyState === XMLHttpRequest.DONE && this.status === 200) {
                    ...
                    answerString = "<table class='table'><thead><tr><th>SeqNo</th><th>Title</th><th>Type</th><th>Date</th><th>Amount:Key</th><th>Side</th><th>Size</th><th>Fee</th></tr></thead><tbody>";
                    for (var key in sortedList) {
                        ...
                        let side;
                        if(sortedList[key]["creator"] === appData.base58AccountAddress) {
                            if(sortedList[key]["recipient"] !== undefined){
                                side = sortedList[key]["recipient"];
                            } else {
                                side = sortedList[key]["creator"];
                            }
                        } else if(sortedList[key]["creator"] === 'genesis') {
                            side = appData.base58AccountAddress;
                        } else {
                            side = sortedList[key]["creator"];
                        }
                        answerString = answerString + "<td><a href='" + appData.nodeUrl + "/index/blockexplorer.html?address=" + side + "&lang=en'>" + side.substring(0, 20) + "..." + "</a></td>";

                        if(sortedList[key]["size"]){
                            answerString = answerString + "<td>" + sortedList[key]["size"] + "</td>";
                        } else {
                            answerString = answerString + "<td>&mdash;</td>";
                        }
