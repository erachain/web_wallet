...
function doAccountFromSeed(base58AccountSeed) {
	...
	if (Base58.decode(base58AccountSeed).length != 32) {
		if (Base58.decode(base58AccountSeed).length != 34) {
	...
	createKeyPair("1RPnosA9t91z6A6CUvSgyURvYozAnEe1xaXmdSHauELpJ");
...
function generateSeedBIP39() {
	...
	var trailer = new Uint8Array([0, 1]);
	var concatTrailerBaseSeed = new Uint8Array(trailer.length + byteSeed.length);
	concatTrailerBaseSeed.set(trailer);
	concatTrailerBaseSeed.set(byteSeed, trailer.length);
	var base58BaseSeed = Base58.encode(concatTrailerBaseSeed);
