	...
	function generatePersonDoPaymentTransaction() {
    	...
    	// person to byte
	    var a;
	    a = toBytePerson(appData.keyPair, head, icon, image , description, birthdayBytes, deathdayBytes, gender, race, birthLatitude, birthLongitude, skinColor, eyeColor, hairColor, height);
	...
	function saveRaw() {
		...
		imagebyte = array;
		$("#img").html('<img src="' + imageData + '" style="max-width:250px; height: AUTO;"/>');
	...
	function generatePersononLoad() {
		...
		hChanger.addEventListener("input", function() {
			...
			$(".image-editor").cropit('previewSize', { width: 250, height: 250 + 85 * Number(hChanger.value) / 100 });
