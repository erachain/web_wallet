...
function generatePersononLoad() {
  ...
  $(function() {
    ...
    $("#submitImage").click(function() {
      ...
      let finalImage = imageCurrentSize();

      if(Array.isArray(finalImage)){

        imagebyte = finalImage[1];

        $("#img").html('<img src="' + finalImage[0] + '" style="max-width:250px; height: AUTO;"/>')
      } else {
        $("#imgwarn").html(
...
function imageCurrentSize(){
  ...
  do {
    ...
    if (imagebyte.length > 15360 && imagebyte.length < 20480) {
      return [imageData, imagebyte];
    }

    quality -= 0.05;

  } while (! current_size && quality !== 0);
}
