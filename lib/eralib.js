...
// encrypt		- 0 not encrypt; 1 encrypt
// is_text		- 1
...
// return byte[]
function generate_R_Send_TransactionBase( keyPair, recipient, asset_key, amount, scale, timestamp, title, message, encrypt, is_text, port) {
	// type transaction
	// referens
	...
	if (message != null && message !="") {
		...
		if(encrypt[0] === 1){
			arr = EraCrypt.encryptMessage(message, getPublicKeyToAddress(Base58.encode(recipient)), Base58.encode(appData.keyPair.privateKey));
		} else {
		...
		data1 = appendBuffer(data1, arr);
		data1 = appendBuffer(data1, encrypt);
	...
	var data0 = tobyteBasePart(typeBytes, int64ToBytes(timestamp), lastReferenceByte, keyPair.publicKey, fee_c);
...
// keyPair			- pair public key + security key (byte64, byte32)
// timestamp		- Unix timestamp (Long). 1514529622881
// blockHeight		- block height (int)
// transNumber		- transaction number (int)
// port				- ERA network PORT 9046 or dev:9066 (int)
// R_Vouch(keyPair, 1514529622881, 200, 1, 9046)
// return byte
function R_Vouch(keyPair, timestamp, blockHeight, transNumber, port){
	// type transaction
	...
	const fee_c = [0];
	// start part
	let data0 = tobyteBasePart(typeBytes, timestampBytes, lastReferenceByte, keyPair.publicKey, fee_c);
	let data1 = new Uint8Array();
	data1 = appendBuffer(data1, int32ToBytes(blockHeight)); 			// Block Height
	data1 = appendBuffer(data1, int32ToBytes(transNumber)); 			// Transaction number from block
	return toByteEndPart(data0, data1, keyPair.privateKey, port );
}
