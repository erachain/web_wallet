	...
	includeJS('./lib/wallet/inboxMail.js');
	includeJS('./lib/wallet/inboxMail.html.js');
	includeJS('./lib/wallet/inboxTelegram.js');
	includeJS('./lib/wallet/inboxTelegram.html.js');
	includeJS('./lib/wallet/signTransaction.js');
	includeJS('./lib/wallet/signTransaction.html.js');
	...
	class AssetsList {
		...
		getName(_key) {
			...
			if ( !_name ) {
				try {
					var assetDict = this.getAsset(_key);
					_name = assetDict["name"];
					this.list[_key] = _name;
				} catch (e) {
					this.list[_key] = undefined;
				}
			}
		...
		getAsset(_key) {
			try {
				...
				let requestUrl = appData.nodeUrl + "/api/asset/" + _key;
	...
	class AppData {
		...
		getDescriptionHTML(panel_encrypt = false, panel_html = true, panel_md = true) {
			this.selectedDescription = 2;
			let panel = "<div class='panel-message'>";
			panel += "<ul class='nav nav-tabs'>";

			if(panel_html) {
				panel += "<li class='nav-item'><a class='nav-link' data-toggle='tab' href='#html' id='ahtml' onclick='appData.selectedDescription=0'>HTML</a></li>";
			}

			if(panel_md) {
				panel += "<li class='nav-item'><a class='nav-link' data-toggle='tab' href='#md' id='amd' onclick='appData.selectedDescription=1'>MarkDown</a></li>";
			}

			panel += "<li class='nav-item'><a class='nav-link active' data-toggle='tab' href='#text' id='atext' onclick='appData.selectedDescription=2'>Plain text</a></li>";
			panel += "</ul>";

			if(panel_encrypt){
				panel +=
					"<div class='panel-message__encrypted'>" +
					"<input type='checkbox' class='checkbox' id='check-encrypted' checked />" +
					"<label for='check-encrypted'>Encrypt message:</label>" +
					"</div>";
			}
			panel += "</div>";
			...
			if(panel_html) {
				panel += "<div class='tab-pane' id='html'><textarea class='form-control' rows='7' id=description></textarea></div>";
			}

			if(panel_md) {
				panel += "<div class='tab-pane' id='md'><textarea class='form-control' rows='7' id=MDdescription></textarea></div>";
			}

			panel += "<div class='tab-pane active' id='text'><textarea class='form-control' rows='12' id=TEXTdescription></textarea></div>";
			panel += "</div>";
			...
			if ( typeof appData.keyPair !== "undefined" ) {
				switch (this.selectedPage) {
					...
					case "orderCreate":
						...
						orderCreateOnLoad(this.selectedPageParams);
						break;
					case "orderDelete":
					...
					case "inboxMails":
						...
						inboxMailsOnLoad(this.selectedPageParams);
						break;
					case "inboxMail":
						document.getElementById("workFlowMainFrame").innerHTML = inboxMail;
						inboxMailOnLoad(this.selectedPageParams);
						break;
					case "inboxTelegrams":
					...
					case "signTransaction":
						document.getElementById("workFlowMainFrame").innerHTML = signTransaction;
						signTransactionLoad();
						break;
					}
				} else {
				...
				httpRequest.onreadystatechange = function() {
					if (httpRequest.readyState === XMLHttpRequest.DONE && this.status === 200) {
						try {
							...
							if ( type === "sendAsset" || type === "sendAssetWant" || type === "sendAssetHave" ) {
								document.getElementById("senderBalance").value = balanceString;

								let asset_list = new AssetsList();
								let asset_item = asset_list.getAsset(assetId);

								let input;
								if(type === "sendAssetWant") {
									input = document.getElementById('wantammount');
								} else if(type === "sendAssetHave") {
									input = document.getElementById('haveammount');
								} else {
									input = document.getElementById('quantity');
								}

								input.dataset.maxAmount = balanceArray[assetId][0][1];
								input.dataset.amountScale = asset_item.scale;
								if(parseFloat(input.value) > parseFloat(balanceArray[assetId][0][1])){
									input.value = 0;
								}
							}
						} catch (e) { }
	...
	function menuClick(_action, _params = null) {
		if (appData != null) {
			appData.selectedPage = _action;
			appData.selectedPageParams = _params;
			appData.render();
		}
	}
	...
	function doPostProcess() {
		...
		$('#output').html('Processing...');
		$.post( appData.nodeUrl + "/api/broadcast", txRaw , function( data) {
	...
	function doPostProcessTelegram() {
		...
		$('#output').html('Processing...');
		$.post( appData.nodeUrl + "/api/broadcasttelegram", txRaw , function( data) {
			if (data.status == undefined) {	
				$("#output").val("Message:" + data.message);					
			} else {
				$("#output").val("Transaction broadcasted sucessfully.");					
	...
	function getPublicKeyToAddress(address) {
		try {
			var httpRequestName = typeof XMLHttpRequest != 'undefined'
				? new XMLHttpRequest()
				: new ActiveXObject('Microsoft.XMLHTTP');
			var requestNameUrl = appData.nodeUrl + "/api/addresspublickey/" + address;
			httpRequestName.open('GET',requestNameUrl, false);
			httpRequestName.send();

			return httpRequestName.responseText;
		} catch (e) {
		}
	}
	...
	function correctMailContent(content) {
		if (content.charAt(0) === '#') {
			let mdContent = content;

			if (mdContent.charAt(1) === "\n") {
				mdContent = mdContent.substr(2);
			}
	...
	function dateToString(date, utc = false) {
		if (utc) {
		...
		return year + '-' + month + '-' + day + ' ' + hours + ':' + minutes + ':' + seconds + (utc ? ' UTC' : '');
	}

	function parseDataTransactionBE(blockSeq) {
		try {
			let httpRequest = typeof XMLHttpRequest != 'undefined'
				? new XMLHttpRequest()
				: new ActiveXObject('Microsoft.XMLHTTP');
			let requestNameUrl = appData.nodeUrl + "/index/blockexplorer.json?tx=" + blockSeq + "&lang=en";
			httpRequest.open('GET',requestNameUrl, false);
			httpRequest.send();
...
/* //TODO пока не используется, парсится json с блокэксплорера
	function parseDataTransaction(transaction) {
		switch (transaction['type']) {
			case 35:
				return parseDataSignNote(transaction);
			default:
				return null;
		}
	}

	function parseDataSignNote(transaction) {
		const DATA_VERSION_LENGTH = 6;
		const DATA_TITLE_SIZE_LENGTH = 4;
		const DATA_JSON_SIZE_LENGTH = 4;

		let dataByte = toUTF8Array(transaction['data']);
		let position = 0;
		let maxPosition = dataByte.length;
		let object = {};

		object['version'] = fromUTF8Array(dataByte.slice(position, DATA_VERSION_LENGTH));
		position += DATA_VERSION_LENGTH;

		const titleLengthByte = dataByte.slice(position, position + DATA_TITLE_SIZE_LENGTH);
		const titleLength = intFromBytes(titleLengthByte);
		position += DATA_TITLE_SIZE_LENGTH;

		object['title'] = fromUTF8Array(dataByte.slice(position, position + titleLength));
		...
		const jsonLengthByte = dataByte.slice(position, position + DATA_JSON_SIZE_LENGTH);
		const jsonLength = intFromBytes(jsonLengthByte);
		position += DATA_JSON_SIZE_LENGTH;
		try {
			object['json'] = JSON.parse(fromUTF8Array(dataByte.slice(position, position + jsonLength)));
		} catch (e) {
			console.log(e);
		}
