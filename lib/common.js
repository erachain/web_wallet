	...
	class AppData {
		...
		getDescriptionValue() {
			...
			if ( this.selectedDescription == 0 ) {
				...
				answerValue = document.getElementById('TEXTdescription');
			}
			return answerValue;
		}
		...
		render() {
			if ( typeof appData.base58AccountSeed !== "undefined" ) {
				switch (this.selectedPage) {
					...
					document.getElementById("workFlowMainFrame").innerHTML = createAddress;
					document.getElementById("weburl").value = appData.nodeUrl;
					if ( this.selectedPage !== "main" && this.selectedPage !== "mainRm" ) {
						alert("Login first.");
					}
				}
